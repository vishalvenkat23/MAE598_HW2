...
def grad(x):
    return [10*x[0] + 12*x[1] - 8, 12*x[0] + 20*x[1] - 14]  # correct gradient!
...
error = np.linalg.norm(grad(x), ord=None, axis=None)
# error = abs(grad(x))
...
def line_search(x):
    ...
    d = - grad(x)
    def phi(a, x):
        return f(x) - a * 0.8 * (np.transpose(grad(x)) * d  # define phi as a search criterion
    while phi(a) < f(x+a*d):  # if f(x+a*d)>phi(a) then backtrack. d is the search direction
        a = 0.5*a
    return a
